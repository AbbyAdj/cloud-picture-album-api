...
class TestDeleteUserPictureEndpoint:
    ...
    def test_returns_200_response(
        self, test_client: TestClient, s3_delete_object_mock_success: Mock
    ):
    ...
    def test_deletes_record_from_database(
        self, test_client: TestClient, s3_delete_object_mock_success: Mock
    ):
    ...
    def test_handles_error(
        self, test_client: TestClient, s3_delete_object_mock_error: Mock
    ):
        user_id = 12
        picture_id = 14
        endpoint = f"/users/{user_id}/pictures/{picture_id}"
...
class TestDeleteUserAlbumEndpoint:
    ...
    def test_returns_200_response(
        self, test_client: TestClient, s3_delete_album_mock_success: Mock
    ):
        response = test_client.delete(self.endpoint)
    ...
    def test_deletes_record_from_database(
        ...
        response = util_funcs["user_album_details"](self.user_id, self.album_id)[
            "album"
        ]
    ...
    def test_handles_error(
        self, test_client: TestClient, s3_delete_album_mock_error: Mock
    ):
        user_id = 12
        album_id = 14
        endpoint = f"/users/{user_id}/albums/{album_id}"
