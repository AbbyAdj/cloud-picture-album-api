...
class TestReturnAllPictures:
    ...
    def test_returns_all_pictures(self):
        ...
        assert first_record["date_created"] == datetime.date(2025, 5, 12)
        ...
        assert last_record["date_created"] == datetime.date(2025, 5, 12)

    def test_returns_correct_datatype(self):
        for record in self.all_pictures_list:
            assert isinstance(record["user_id"], int)
            assert isinstance(record["album_id"], int)
            assert isinstance(record["user_first_name"], str)
            assert isinstance(record["user_last_name"], str)
            ...
            assert isinstance(record["date_created"], datetime.date)
...
class TestReturnPicture:
    ...
    def test_returns_one_picture_(self):
        ...
        assert self.record["date_created"] == datetime.date(2025, 5, 12)
    ...
    def test_returns_correct_datatype(self):
        ...
        assert isinstance(self.record["date_created"], datetime.date)
        assert isinstance(self.record["s3_key_name"], str)
        assert isinstance(self.record["picture_description"], str | None)
...
class TestReturnUserDetails:
    ...
    def test_returns_correct_number_of_records(self):
        # Num of rows
        assert len(self.user_details["user"]) == 1
...
class TestReturnUserAlbums:
    user_albums = util_funcs["user_albums"](user_id=1)
...
class TestReturnUserAlbumDetails:
    ...
    def test_returns_all_user_album_details(self):
        assert self.record["album_name"] == "default"
        assert self.record["album_s3_path"] == "user-1"
        assert self.record["picture_name"] == "pic1"
        assert self.record["picture_s3_path"] == "user-1/pic1"
        assert self.record["picture_description"] == None
        assert self.record["date_created"] == datetime.date(2025, 5, 12)
...
class TestReturnAllUserPictures:
    ...
    def test_returns_all_user_pictures(self):
        ...
        assert self.first_record["date_created"] == datetime.date(2025, 5, 12)
        assert self.first_record["picture_description"] == None

        assert self.last_record["picture_name"] == "pic6"
        assert self.last_record["album_name"] == "clothes"
        assert self.last_record["picture_s3_path"] == "user-4/clothes/pic6"
        assert self.last_record["date_created"] == datetime.date(2025, 5, 12)
        assert self.last_record["picture_description"] == None

    def test_returns_correct_datatype(self):
        for record in self.all_user_pictures_list:
            assert isinstance(record["picture_name"], str)
            assert isinstance(record["album_name"], str)
            assert isinstance(record["picture_s3_path"], str)
            assert isinstance(record["date_created"], datetime.date)
...
class TestInsertNewPicture:
    new_picture_details = PostPictureModel(
        picture_name="new-pic", picture_description="new input"
    )
    ...
    def test_inserts_new_picture(self):
        ...
        s3_response = {
            ...
            "s3_key_name": key,
            "date_created": datetime.date(2025, 5, 6),
        ...
        new_picture = util_funcs["insert_new_picture"](user_id, album_id, s3_response)[
            "picture"
        ][0]
        ...
        assert new_picture["date_created"] == datetime.date(2025, 5, 6)
        assert new_picture["s3_key_name"] == key
        assert (
            new_picture["picture_description"]
            == self.new_picture_details.picture_description
        )
    ...
    def test_handles_errors_from_s3_upload(self):
        ...
        s3_response = {
            "error": "Upload unsuccessful. Try again later",
            "details": "arbirtray error",
        }

        response = util_funcs["insert_new_picture"](user_id, album_id, s3_response)
...
class TestDeleteUserPicture:
    ...
    def test_deletes_user_picture(self):
        ...
        user_pictures_before_deletion = util_funcs["all_user_pictures"](user_id)[
            "pictures"
        ]

        new_picture = util_funcs["delete_user_picture"](
            user_id, picture_id, s3_response
        )

        pictures_after_deletion = util_funcs["all_pictures"]()["pictures"]
        user_pictures_after_deletion = util_funcs["all_user_pictures"](user_id)[
            "pictures"
        ]

        assert len(pictures_after_deletion) == len(pictures_before_deletion) - 1
        assert (
            len(user_pictures_after_deletion) == len(user_pictures_before_deletion) - 1
        )

        assert (
            new_picture["success"]
            == f"User {user_id}'s picture with picture id {picture_id} deleted successfully"
        )
    ...
    def test_handles_errors_from_s3_delete(self):
        ...
        new_picture = util_funcs["insert_new_picture"](user_id, picture_id, s3_response)
...
class TestDeleteUserAlbum:
    ...
    def test_deletes_user_album(self):
        ...
        user_albums_before_deletion = util_funcs["user_albums"](user_id)["albums"]

        response = util_funcs["delete_user_album"](user_id, album_id, s3_response)

        albums_after_deletion = util_funcs["all_albums"]()["albums"]
        user_albums_after_deletion = util_funcs["user_albums"](user_id)["albums"]
        ...
        assert (
            response["success"]
            == f"User {user_id}'s album with album id {album_id} deleted successfully"
        )
    ...
    def test_recursively_deletes_user_pictures_in_album(self):
        ...
        user_pictures_in_album = len(
            util_funcs["user_album_details"](user_id, album_id)["album"]
        )

        user_pictures_before_deletion = len(
            util_funcs["all_user_pictures"](user_id)["pictures"]
        )

        util_funcs["delete_user_album"](user_id, album_id, s3_response)

        user_pictures_after_deletion = len(
            util_funcs["all_user_pictures"](user_id)["pictures"]
        )

        assert (
            user_pictures_before_deletion
            == user_pictures_in_album + user_pictures_after_deletion
        )
